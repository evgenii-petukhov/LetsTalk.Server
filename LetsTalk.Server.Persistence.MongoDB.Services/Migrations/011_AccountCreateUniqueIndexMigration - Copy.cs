...
public class AccountCreateUniqueIndexMigration : IMigration
{
    ...
    public string Name => "Account: Create a unique index (AccountTypeId + ExternalId)";
    ...
    public void Up(IMongoDatabase database)
    {
        database.GetCollection<Account>(nameof(Account)).Indexes.CreateOne(new CreateIndexModel<Account>(
            Builders<Account>.IndexKeys.Ascending(x => x.AccountTypeId).Ascending(x => x.ExternalId),
